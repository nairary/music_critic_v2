...
PHASE8A_CPU_CUDA_NUMERICAL_PARITY_RTOL = 1.0e-3
PHASE8A_CPU_CUDA_NUMERICAL_PARITY_ATOL = 5.0e-5
PHASE8A_CPU_CUDA_MIN_COSINE_SIMILARITY = 0.999
...
def _validate_exact_final_source(
    *,
    expected_head: str,
) -> tuple[str, bool]:
...
def _graphs_cross_device_bit_exact(left: Any, right: Any) -> bool:
    if (
        tuple(left.node_types) != tuple(right.node_types)
        or tuple(left.edge_types) != tuple(right.edge_types)
    ):
        return False
    left_stores = _store_items(left)
    right_stores = _store_items(right)
    if tuple(name for name, _ in left_stores) != tuple(
        name for name, _ in right_stores
    ):
        return False
    for (left_name, left_store), (right_name, right_store) in zip(
        left_stores,
        right_stores,
        strict=True,
    ):
        ...
        for key in left_keys:
            left_value = left_store[key]
            right_value = right_store[key]
            if isinstance(left_value, Tensor) or isinstance(
                right_value,
                Tensor,
            ):
                if (
                    not isinstance(left_value, Tensor)
                    or not isinstance(right_value, Tensor)
                    ...
                    return False
            elif _metadata_signature(left_value) != _metadata_signature(
                right_value
            ):
                return False
    return True
...
def _tensor_numerical_parity_evidence(
    pairs: Mapping[str, tuple[Tensor, Tensor]],
    *,
    rtol: float = PHASE8A_CPU_CUDA_NUMERICAL_PARITY_RTOL,
    atol: float = PHASE8A_CPU_CUDA_NUMERICAL_PARITY_ATOL,
    min_cosine_similarity: float = (
        PHASE8A_CPU_CUDA_MIN_COSINE_SIMILARITY
    ),
) -> dict[str, object]:
    """Measure bounded FP32 backend differences without weakening exact gates."""

    if not pairs:
        raise ValueError("numerical parity requires at least one tensor pair")
    if rtol < 0.0 or atol < 0.0:
        raise ValueError("numerical parity tolerances must be non-negative")
    ...
    for name, (cpu_value, backend_value) in pairs.items():
        shape_exact = cpu_value.shape == backend_value.shape
        dtype_exact = (
            cpu_value.dtype == backend_value.dtype == torch.float32
        )
        shapes_exact = shapes_exact and shape_exact
        dtypes_exact = dtypes_exact and dtype_exact
        if not shape_exact:
            per_tensor[name] = {
                "shape_exact": False,
                "dtype_exact": dtype_exact,
                "finite": False,
                "max_abs_cpu_cuda_difference": None,
                "max_rel_cpu_cuda_difference": None,
            ...
            all_finite = False
            continue
        left = cpu_value.detach().to(device="cpu", dtype=torch.float32)
        right = backend_value.detach().to(
            device="cpu",
            dtype=torch.float32,
        )
        finite = bool(torch.isfinite(left).all()) and bool(
        ...
        denominator = torch.maximum(
            ...
            torch.full_like(left, torch.finfo(torch.float32).tiny),
        )
        tensor_max_abs = (
        ...
        if left.numel() == 0 or torch.equal(left, right):
            cosine = 1.0
        else:
        ...
        close = (
            ...
            and bool(
                torch.all(
                    difference
                    <= atol + rtol * torch.abs(left)
                )
            )
        )
        parity_passed = close and cosine >= min_cosine_similarity
        max_abs = max(max_abs, tensor_max_abs)
        max_rel = max(max_rel, tensor_max_rel)
        minimum_cosine = min(minimum_cosine, cosine)
        compared_element_count += left.numel()
        all_close = all_close and parity_passed
        per_tensor[name] = {
            "shape_exact": True,
            "dtype_exact": dtype_exact,
            "finite": finite,
            "max_abs_cpu_cuda_difference": tensor_max_abs,
            "max_rel_cpu_cuda_difference": tensor_max_rel,
    ...
    return {
        ...
        "configured_min_cosine_similarity": min_cosine_similarity,
        "compared_tensor_count": len(pairs),
        "compared_element_count": compared_element_count,
        "max_abs_cpu_cuda_difference": max_abs,
        "max_rel_cpu_cuda_difference": max_rel,
        "minimum_cosine_similarity": minimum_cosine,
        "shapes_exact": shapes_exact,
        "dtypes_exact": dtypes_exact,
        "finite_status_exact": all_finite,
        "cross_backend_parity_passed": passed,
        "per_tensor": per_tensor,
    }


def _required_loss_tensors(output: Any) -> dict[str, Tensor]:
...
def _cross_backend_numerical_parity(
    cpu_output: Any,
    cuda_output: Any,
) -> dict[str, object]:
    node_pairs = {
        node_type: {
            f"{node_type}.online_embedding": (
                cpu_output.online_encoder.fused.embeddings[node_type],
                cuda_output.online_encoder.fused.embeddings[node_type],
            )
        }
        for node_type in cpu_output.online_encoder.fused.embeddings
    }
    if tuple(node_pairs) != tuple(
        cuda_output.online_encoder.fused.embeddings
    ):
        raise RuntimeError(
            "phase8a.cuda.cross_backend_node_types_mismatch"
        )
    per_node_type = {
        node_type: _tensor_numerical_parity_evidence(pairs)
        for node_type, pairs in node_pairs.items()
    }
    prediction_pairs = {
        f"decoder_prediction_{index}": pair
    ...
    target_pairs = {
        level: (
            ...
            getattr(cuda_output.targets, level),
        )
        for level in ("note", "bar", "song")
    }
    cpu_losses = _required_loss_tensors(cpu_output)
    cuda_losses = _required_loss_tensors(cuda_output)
    if tuple(cpu_losses) != tuple(cuda_losses):
        raise RuntimeError(
            "phase8a.cuda.cross_backend_loss_surface_mismatch"
        )
    loss_pairs = {
        name: (cpu_losses[name], cuda_losses[name])
        for name in cpu_losses
    }
    all_pairs = {
        **{
            name: pair
            for pairs in node_pairs.values()
            for name, pair in pairs.items()
        },
        **prediction_pairs,
        **{f"target_{name}": pair for name, pair in target_pairs.items()},
        **{f"loss_{name}": pair for name, pair in loss_pairs.items()},
    }
    aggregate = _tensor_numerical_parity_evidence(all_pairs)
    prediction_evidence = _tensor_numerical_parity_evidence(
        prediction_pairs
    )
    target_evidence = _tensor_numerical_parity_evidence(target_pairs)
    loss_evidence = _tensor_numerical_parity_evidence(loss_pairs)
    total_loss_difference = abs(
        float(cpu_losses["total"].detach().cpu())
        - float(cuda_losses["total"].detach().cpu())
    )
    passed = aggregate["cross_backend_parity_passed"] is True and all(
        evidence["cross_backend_parity_passed"] is True
        for evidence in per_node_type.values()
    )
    report = {
        **{
            key: aggregate[key]
            for key in (
                "configured_rtol",
                "configured_atol",
                "configured_min_cosine_similarity",
                "compared_tensor_count",
                "compared_element_count",
                "max_abs_cpu_cuda_difference",
                "max_rel_cpu_cuda_difference",
        ...
        "objective_difference": total_loss_difference,
        ...
        "cross_backend_parity_passed": passed,
        "interpretation": (
            "bounded CPU FP32 versus CUDA FP32 numerical diagnostic; "
            "not evidence of identical floating-operation implementations"
        ),
    }
    if not passed:
        raise RuntimeError(
            "phase8a.cuda.cross_backend_numerical_parity_failed:"
            f"max_abs={report['max_abs_cpu_cuda_difference']}:"
            f"max_rel={report['max_rel_cpu_cuda_difference']}"
        )
    return report
...
def _loss_evidence(
    ...
    *,
    ...
    resolved = _required_loss_tensors(output)
    if not all(
...
def _policy_acceptance(
    ...
    *,
    ...
    cpu_batch, moved_cpu_binding = move_ssl_batch_with_prepared_binding(
        source_batch,
        binding,
        "cpu",
    )
    ...
    if moved_cpu_binding.to_dict() != moved_binding.to_dict():
        raise RuntimeError(
            "phase8a.cuda.cross_backend_binding_mismatch"
        )
    if not _graphs_cross_device_bit_exact(
        cpu_batch.raw_graph_batch,
        batch.raw_graph_batch,
    ):
        raise RuntimeError(
            "phase8a.cuda.cross_backend_raw_graph_mismatch"
        )
    ...
    with torch.no_grad():
        cpu_fp32 = cpu_model.forward_hierarchy(
            cpu_batch,
            prepared_mask_binding=moved_cpu_binding,
        )
        cuda_fp32 = model.forward_hierarchy(
            batch,
            prepared_mask_binding=moved_binding,
        )
        cuda_fp32_repeated = model.forward_hierarchy(
            batch,
            prepared_mask_binding=moved_binding,
        )
    ...
    if (
        ...
        != cuda_fp32.feature_overlay.to_dict()
        or not torch.equal(
            cpu_fp32.selected_global_note_indices,
            cuda_fp32.selected_global_note_indices.cpu(),
        )
    ):
        raise RuntimeError(
            "phase8a.cuda.cross_backend_exact_invariant_mismatch"
        )
    cross_backend_parity = _cross_backend_numerical_parity(
        cpu_fp32,
        cuda_fp32,
    )
    ...
    policy_report = {
        ...
        "same_device_cuda_fp32_repeat_bit_exact": True,
        "cross_backend_exact_invariants": {
            "prepared_binding": True,
            "mask_plans": True,
            "feature_overlay_and_masks": True,
            "selected_global_note_indices": True,
            "raw_graph_and_topology": True,
        },
        "cross_backend_numerical_parity": cross_backend_parity,
