...
def _write_compound_midi(
    path: Path,
    *,
    onset_ticks: int = 0,
    duration_ticks: int = 2,
    meter_events: tuple[tuple[int, int, int], ...] = ((0, 6, 8),),
    ...
    previous_tick = 0
    for tick, numerator, denominator in meter_events:
        conductor.append(
            mido.MetaMessage(
                "time_signature",
                numerator=numerator,
        ...
        previous_tick = tick
    conductor.append(mido.MetaMessage("end_of_track", time=6 - previous_tick))
    ...
    if include_note:
        ...
        notes.append(mido.MetaMessage("end_of_track", time=6))
...
def _write_canonical_projection(render_dir: Path, clip_id: str) -> None:
    ...
    for message in midi.tracks[0]:
        ...
        if message.type == "time_signature":
            meter_events.append(
                {
                    "onset_qn": {"num": tick, "den": midi.ticks_per_beat},
...
def _set_canonical_meter_onset(
    render_dir: Path, clip_id: str, index: int, *, numerator: int, denominator: int
) -> None:
    path = render_dir / f"{clip_id}.canonical.json"
    document = json.loads(path.read_text(encoding="utf-8"))
    document["meter_events"][index]["onset_qn"] = {
        "num": numerator,
        "den": denominator,
    }
...
def _comparison_case(
    ...
    *,
    ...
    reference_meters: list[dict] | None = None,
    midi_meter_events: tuple[tuple[int, int, int], ...] = ((0, 6, 8),),
    include_note: bool = True,
) -> tuple[Path, Path]:
    clip_id = "case"
    simplified_path = tmp_path / "Hooktheory.json"
        ...
        json.dumps(
            {
                clip_id: {
                    "annotations": {
                        ...
                        or [{"beat": 0, "beats_per_bar": 6, "beat_unit": 8}],
                        "melody": [
                            {
    ...
    _write_compound_midi(
        ...
        meter_events=midi_meter_events,
...
def test_quantized_simplified_meter_onset_within_half_tick_is_accepted(
    tmp_path: Path,
) -> None:
    simplified, render_dir = _comparison_case(
        tmp_path,
        ...
        reference_meters=[
            {"beat": 0, "beats_per_bar": 6, "beat_unit": 8},
            {"beat": 0.6, "beats_per_bar": 3, "beat_unit": 4},
        ],
        midi_meter_events=((0, 6, 8), (1, 3, 4)),
        include_note=False,
    )
    ...
    comparison = report["comparisons"][0]
    assert comparison["meter_regions_exact"] is False
    assert comparison["meter_regions_accepted"] is True
    assert comparison["meter_status"] == "quantized"
    assert comparison["symbolic_accepted"] is True
    assert comparison["audit_passed"] is True
    assert report["meter_regions_exact_clips"] == 0
    assert report["meter_regions_accepted_clips"] == 1
    assert report["meter_regions_quantization_accepted_clips"] == 1
    assert report["meter_mismatch_clips"] == 0
    assert main(
        [
            "--simplified-path",
            str(simplified),
...
def test_simplified_meter_onset_above_half_tick_is_rejected(tmp_path: Path) -> None:
    simplified, render_dir = _comparison_case(
        tmp_path,
        reference_onset=0,
        reference_offset=0,
        ...
        reference_meters=[
            {"beat": 0, "beats_per_bar": 6, "beat_unit": 8},
            {"beat": 0.2, "beats_per_bar": 3, "beat_unit": 4},
        ],
        midi_meter_events=((0, 6, 8), (1, 3, 4)),
        include_note=False,
    )
    ...
    comparison = report["comparisons"][0]
    assert comparison["meter_regions_exact"] is False
    assert comparison["meter_regions_accepted"] is False
    assert comparison["meter_status"] == "disagrees"
    assert report["meter_mismatch_clips"] == 1
    assert main(
        [
            "--simplified-path",
            str(simplified),
...
def test_exact_mode_rejects_nonzero_simplified_and_canonical_meter_onset(
    tmp_path: Path,
) -> None:
    simplified, render_dir = _comparison_case(
        tmp_path,
    ...
    _set_canonical_meter_onset(
        render_dir, "case", 1, numerator=3, denominator=10
    )
    ...
    comparison = report["comparisons"][0]
    assert comparison["meter_regions_accepted"] is False
    assert comparison["canonical_meter_events_accepted"] is False
    assert "exact_meter_onset_error_nonzero" in comparison["independent_timing_violations"]
    assert comparison["audit_passed"] is False
    assert main(
        [
            "--simplified-path",
...
def test_simplified_meter_signature_mismatch_is_rejected(tmp_path: Path) -> None:
    simplified, render_dir = _comparison_case(
        tmp_path,
        reference_onset=0,
        reference_offset=0,
        ...
        reference_meters=[
            {"beat": 0, "beats_per_bar": 6, "beat_unit": 8},
            {"beat": 1, "beats_per_bar": 3, "beat_unit": 4},
        ],
        midi_meter_events=((0, 6, 8), (1, 4, 4)),
        include_note=False,
    )
